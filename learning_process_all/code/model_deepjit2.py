...
from dataset import CodeAndMsgDataset
...
class ModelDeepJIT2(Model):
    ...
    def fit(self, train_loader, valid_loader, optimizer, criterion, earlystopping, scheduler=None):
        ...
        for epoch in range(self.params['epochs']):
            ...
            self.model.train()
            for code_features, msg_features ,labels in tqdm(train_loader):
            ...
            self.model.eval()
            with torch.no_grad():
                for val_code_features, val_msg_features, val_labels in valid_loader:
                    ...
                    val_outputs = self.model([val_code_features, val_msg_features])
