        ...
        for file in commit['codes']:
            ...
            for key, item in list(file.items())[1:]:
                ...
                if item != []:
                    ...
                    lines = [processor.process_code(line, file['filepath']) for line in item]
                    source = filepath + ' <'+key+'> ' + ' <sep> '.join(lines) + ' <sep>'
                commit_dict['codes'].append(source)
                # commit_dict['codes'].extend(lines)
                # print(commit_dict['codes'])
        commit_dict['num_added'] = num_added
        commit_dict['num_deleted'] = num_deleted
...
def read_args():
    ...
    parser.add_argument('-code_user_defined_symbols', type=list, default=['<pad>', '<num>','<literal>','<added_code>','<deleted_code>', '<sep>'])
    parser.add_argument('-msg_user_defined_symbols', type=list, default=['<pad>', '<num>'])
