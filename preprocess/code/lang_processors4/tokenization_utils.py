import re
from sacrebleu.tokenizers.tokenizer_intl import TokenizerV14International

# IMPORTED
NEWLINE_TOKEN = "NEWLINE_TOKEN"


# IMPORTED
class ind_iter(object):
    def __init__(self, len):
        self.i = 0
        self.len = len

    def next(self):
        self.i += 1
        if self.i > (self.len - 1):
            raise StopIteration

    def prev(self):
        self.i -= 1
        if self.i < 0:
            raise StopIteration


# IMPORTED
def process_string(tok, char2tok, tok2char, is_comment, do_whole_processing=True):
    if not (do_whole_processing or is_comment):
        return tok.replace("\n", "\\n").replace("\r", "")

    if is_comment:
        tok = re.sub(" +", " ", tok) #複数の空白を1つの空白に置き換える
        tok = re.sub(r"(.)\1\1\1\1+", r"\1\1\1\1\1", tok) #同じ文字が５回以上繰り返されるパターンを５回に置換する
        if len(re.sub(r"\W", "", tok)) < 2: #文字列から
            return ""

    tok = replace_general_string_tok(tok)
    tok = replace_tokens(tok, char2tok)
    if tok.strip().startswith("STOKEN00"): # #コメントの処理？
        if " STRNEWLINE " in tok:
            tok = tok.replace(" STRNEWLINE ", " ENDCOM", 1)
        else:
            tok += " ENDCOM"
    if not do_whole_processing:
        tok = replace_tokens(
            tok, {f" {key} ": value for key, value in tok2char.items()}
        )
        tok = (
            tok.replace(" ▁ ", " ")
                .replace(" TABSYMBOL ", "\t")
                .replace("\\r", "")
                .replace(" STRNEWLINE ", "\\n")
        )
        return tok
    tok = re.sub(" +", " ", tok)
    tokenize = TokenizerV14International()
    tok = tokenize(tok)
    tok = re.sub(" +", " ", tok)
    tok = tok.replace("\r", "")
    tok = replace_tok_general_string(tok)
    for special_token, char in tok2char.items():
        tok = tok.replace(special_token, char)
    return tok


def tokenize_string(s: str):
    return process_string(
        s, char2tok=dict(), tok2char=dict(), is_comment=False, do_whole_processing=True
    ).split(" ")


def detokenize_string(s):
    assert isinstance(s, str) or isinstance(s, list)
    if isinstance(s, list):
        s = " ".join(s)
    return s.replace(" ", "").replace("▁", " ")


# IMPORTED
def replace_tokens(tok, dictionary):
    for char, special_token in dictionary.items():
        tok = tok.replace(char, special_token)
    return tok


# IMPORTED
def replace_general_string_tok(tok):
    return (
        tok.replace(" ", " ▁ ")
            .replace("\n", " STRNEWLINE ")
            .replace("\t", " TABSYMBOL ")
    )

def replace_tok_general_string(tok):
    return (
        tok.replace("STRNEWLINE", "\n")
            .replace("TABSYMBOL","\t")
    )

# def replace_tok_general_string(tok):
#     return (
#         tok.replace("STRNEWLINE", "\n")
#             .replace("TABSYMBOL","\t")
#             .replace(" ", "")
#             .replace("▁", " ")
#     )


# IMPORTED
def indent_lines(lines):
    prefix = ""
    for i, line in enumerate(lines):
        line = line.strip()
        if re.match("CB_COLON|CB_COMA|CB_", line):
            prefix = prefix[2:]
            line = prefix + line
        elif line.endswith("OB_"):
            line = prefix + line
            prefix += "  "
        else:
            line = prefix + line
        lines[i] = line
    untok_s = "\n".join(lines)
    return untok_s
