...
def integrate_line(project, processor):
    path = '../resource/preprocess_data/openstack_replace/'+project
    ...
    for i, commit in enumerate(project_data):
        ...
        for file in commit['codes']:
            ...
            for key, item in list(file.items())[1:]:
                if item != []:
                    lines = [processor.process(line) for line in item]
                    source =  check_ext(file['filepath']) + ' <sep> '.join(lines) + ' <sep>'
...
def read_args():
    ...
    parser.add_argument('-model_prefix', type=str, default='openstack_spm_1000')
    parser.add_argument('-vocab_size', type=int, default=1000)
    parser.add_argument('-character_coverage', type=int, default=1.0)
    parser.add_argument('-user_defined_symbols', type=list, default=['<pad>','<cpp>','<java>','<python>','<sep>','<num>','<str>'])
