...
class LVMVolumeDriver(driver.VolumeDriver):
    ...
    def _delete_volume(self, volume, size_in_g):
        ...
        self._try_execute(
            "lvremove",
            "-f",
            "%s/%s" % (FLAGS.volume_group, self._escape_snapshot(volume["name"])),
            run_as_root=True,
        )
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        self._create_volume(volume["name"], self._sizestr(volume["size"]))
        self._copy_volume(
            self.local_path(snapshot), self.local_path(volume), snapshot["volume_size"]
        )
        ...
        if FLAGS.volume_clear == "zero":
            if size_in_m == 0:
                return self._copy_volume(
                    "/dev/zero", vol_path, size_in_g, clearing=True
                )
            else:
            ...
            LOG.error(
                _("Error unrecognized volume_clear option: %s"), FLAGS.volume_clear
            )
            return
...
class LVMISCSIDriver(LVMVolumeDriver, driver.ISCSIDriver):
    ...
    def ensure_export(self, context, volume):
        ...
        if (
            volume["provider_location"] is not None
        ...
        self.tgtadm.create_iscsi_target(
            iscsi_name,
            iscsi_target,
            0,
            volume_path,
    ...
    def _fix_id_migration(self, context, volume):
        ...
        volume["provider_location"] = volume["provider_location"].replace(
    ...
    def create_export(self, context, volume):
        ...
        chap_auth = self._iscsi_authentication(
            "IncomingUser", chap_username, chap_password
        )
        # NOTE(jdg): For TgtAdm case iscsi_name is the ONLY param we need
        # should clean this all up at some point in the future
    ...
    def remove_export(self, context, volume):
        ...
        try:
            ...
            LOG.info(
                _(
                    "Skipping remove_export. No iscsi_target "
                    "is presently exported for volume: %s"
                ),
                volume["id"],
            )
            return
