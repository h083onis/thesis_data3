    ...
    for i, commit in enumerate(project_data):
        if not isinstance(df, pd.DataFrame):
            if i < len(project_data) - test_rate * len(project_data):
                commit['kinds'] = 'train'
                lines_list.extend(commit['codes'])
            else:
                commit['kinds'] = 'test'
        else:
...
def integrate_line(project, processor):
    path = '../resource/preprocess_data/openstack_idf_tokenize/'+project
        ...
        for file in commit['codes']:
            for key, item in list(file.items())[1:]:
                if item != []:
                    ...
                    lines = [tokenizer(file['filepath']) + ' <'+key+'> '+ processor.process(line) for line in item]
...
def read_args():
    ...
    parser.add_argument('-user_defined_symbols', type=list, default=['<num>','<literal>','<added_code>','<deleted_code>'])
