...
from clean_comment import remove_c_com_and_doc, remove_py_com_and_doc
...
def print_code(repo, hexsha, filepath, ext, type):
    ...
    if ext == 'py':
        output = remove_py_com_and_doc(output)
        if output == False:
            return False
    else:
    ...
    for repo_name in repo_list:  
        ...
        for i, hexsha in enumerate(id_list):
            ...
            try:
                ...
                pipe_process(repo, commit, hexsha, params, commit_dict, logger, type='normal')
            except (IndexError, BadName):   
                pipe_process(repo, commit, hexsha, params, commit_dict, logger, type='first')
