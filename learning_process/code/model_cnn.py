...
class ModelCNN(Model):
    ...
    def predict(self, te_dataset):
        self.model.eval()
        with torch.no_grad():
            features_test = torch.LongTensor(te_dataset[0]).to(self.device)
            outputs_test = self.model(features_test)
        del self.model
    ...
    def fit(self, tr_loader, va_loader, optimizer, criterion, earlystopping):
        ...
        for epoch in range(self.params['epochs']):
            ...
            #訓練フェーズ
            self.model.train()
            for features, labels in tqdm(tr_loader):
                # 1バッチあたりのデータ件数
            ...
            self.model.eval()
            with torch.no_grad():
                for features_valid, labels_valid in va_loader:
                    # 1バッチあたりのデータ件数
                    ...
                    # 予測計算
                    outputs_valid = self.model(features_valid)
    ...
    def cal_auc(self):
        with torch.no_grad():
            outputs_test = self.model(self.va_dataset[0])
