...
class ModelBert(Model):
    def train(self, filepath, train_idx, valid_idx, kind='train'):
        ...
        dataset.tokenizer.save_pretrained('../resource/'+self.run_fold_name+'.tk')
        ...
        self.model.resize_token_embedding(len(dataset.tokenizer))
    ...
    def predict(self, filepath, kind='test'):
        ...
        dataset.tokenizer.from_pretrained('../resource/'+self.run_fold_name+'.tk')
        ...
        self.model.eval()
        with torch.no_grad():
            for inputs_ids, attention_masks, labels in test_loader:
                inputs_ids = inputs_ids.to(self.device)
                attention_masks = attention_masks.to(self.device)
                outputs = self.model(inputs_ids, token_type_ids=None, attention_mask=attention_masks)
    ...
    def fit(self, train_loader, valid_loader, optimizer, criterion, earlystopping, scheduler=None):
        ...
        for epoch in range(self.params):
            ...
            for inputs_ids, attention_masks, labels in tqdm(train_loader):
            ...
            with torch.no_grad():
                for val_code_features, val_msg_features, val_metrics_features, val_labels in valid_loader:
                    ...
                    val_outputs = self.model(inputs_ids, token_type_ids=None, attention_mask=attention_masks)
